...
filter(Cadiz16, Cadiz16[,1]>194 & Cadiz16[,1]<202) %>% kable()
filter(SalvadorAbril, SalvadorAbril[,1]>122&SalvadorAbril[,1]<130) %>% kable()
```
...
for (t in calles){
  primera_fecha=c(primera_fecha,min(t$dateObserved))
}
inicio_calle = data.frame(names(calles), primera_fecha %>% as.Date())
inicio_calle
```
