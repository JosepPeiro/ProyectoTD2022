...
```{r}
datos_wide = datos %>% pivot_wider(names_from=noise, values_from=value)
cat('Covarianza Pearson\n')
cov(datos_wide[,7:11] %>% na.omit())
cat('\nCorrelación Pearson\n')
cor(datos_wide[,7:11] %>% na.omit())
cat('\nCovarianza Spearman\n')
cov(datos_wide[,7:11] %>% na.omit(),method = 'spearman')
cat('\nCorrelación Spearman\n')
cor(datos_wide[,7:11] %>% na.omit(),method = 'spearman')

GGally::ggpairs(datos_wide[,7:11] %>% na.omit())
chart.Correlation(datos_wide[,7:11] %>% na.omit())
```
