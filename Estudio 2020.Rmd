...
packages = c("readr","dplyr","lubridate", "tidyr","tibble", "ggplot2", "purrr", "PerformanceAnalytics")
...
## Estudio de las matrices de correlación y covarianza

```{r}

cat('Covarianza Pearson\n')
cov(select(datos,starts_with("LAeq")))
cat('\nCorrelación Pearson\n')
cor(select(datos,starts_with("LAeq")))
cat('\nCovarianza Spearman\n')
cov(select(datos,starts_with("LAeq")),method = 'spearman')
cat('\nCorrelación Spearman\n')
cor(select(datos,starts_with("LAeq")),method = 'spearman')

chart.Correlation(select(datos,starts_with("LAeq")))

```

```{r}
datos = datos %>% select(-year) %>% gather("noise", "value", 2:6)
```

```{r}
ggplot(datos, aes(noise, value))+geom_boxplot()
```
