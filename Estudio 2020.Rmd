...
packages = c("readr","dplyr","lubridate", "tidyr","tibble", "ggplot2", "purrr", "PerformanceAnalytics", "grid", "ggridges", "forcats")
...
a1=ggplot(datos_tidy %>% filter(noise=="morning"), aes(x = dateObserved, y = value)) +
  geom_line() + facet_wrap(~Street, nrow=4)
a2=ggplot(datos_tidy %>% filter(noise=="afternoon"), aes(x = dateObserved, y = value)) +
  geom_line() + facet_wrap(~Street, nrow=4)
a3=ggplot(datos_tidy %>% filter(noise=="night"), aes(x = dateObserved, y = value)) +
  geom_line() + facet_wrap(~Street, nrow=4)
a4=ggplot(datos_tidy %>% filter(noise=="all"), aes(x = dateObserved, y = value)) +
  geom_line() + facet_wrap(~Street, nrow=4)
a5=ggplot(datos_tidy %>% filter(noise=="min"), aes(x = dateObserved, y = value)) +
  geom_line() + facet_wrap(~Street, nrow=4)
a1
a2
a3
...
```{r}
datos_estadisticos = datos_tidy %>% group_by(Street) %>% summarise(long, mean_val=mean(value), median_val=median(value), max_val=max(value), min_val=min(value))
...
```{r}
lvtt=ggplot(datos_tidy, aes(long, value, color=Street))+geom_jitter()+scale_y_continuous()+theme(legend.position = "none")
