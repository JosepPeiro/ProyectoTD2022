...
```
```{r}
datos_tidy$level=factor(datos_tidy$level, levels=c("Very Low", "Low", "Medium", "Loud", "Very Loud"))
nr1=datos_tidy %>% ggplot(aes(level,..count..*100/sum(..count..), fill=level))+
  geom_bar()+
  scale_y_continuous(limits = c(0,100))+
  labs(y="Porcentaje por nivel de sonido")
nr2=datos_tidy %>% ggplot(aes(noise, fill=level))+
  geom_bar(position="dodge")+
  theme(legend.position="none")
nr3=datos_tidy %>% ggplot(aes(Street, fill=level))+
  geom_bar(position="dodge")+
