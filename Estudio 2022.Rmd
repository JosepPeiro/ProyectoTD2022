...
```{r}
datos %>% group_by(level) %>% summarise(num=n(), porcentaje=num/nrow(datos))%>% ungroup() %>% kable(caption = "Tabla porcentajes segun nivel de ruido") %>% kable_styling(full_width = F, position = "left")
datos %>% filter(month(dateObserved)!=3)%>% group_by(level) %>% summarise(num=n(), porcentaje=num/nrow(datos %>% filter(month(dateObserved)!=3))) %>% ungroup() %>% kable(caption = "Tabla porcentajes segun nivel de ruido sin contar Marzo") %>% kable_styling(full_width = F, position = "left")
```
