...
```
Los resultados este año son soprendentes porque la correlacion cada vez es más visible entre las distintas variables.
...
Tal y como se observan los valores de correlacion cada vez se puede afirmar con menos incertidumbre que si un día empieza siendo ruidoso acabará siendo ruidoso y viceversa.
...
Vamos a calcular el procentaje de cada uno de los niveles de sonido dentro del conjunto entero de datos, pero también vamos a hacer lo mismo a sustrayendo los valores de marzo, para ver si el reultado cambia mucho.

```{r}
datos %>% group_by(level) %>% summarise(num=n(), porcentaje=num/nrow(datos))%>% ungroup()
datos %>% filter(month(dateObserved)!=3)%>% group_by(level) %>% summarise(num=n(), porcentaje=num/nrow(datos %>% filter(month(dateObserved)!=3))) %>% ungroup() 
```
...
-¿Existe algún dato nulo?
-¿Tras ver los datos de este año, apreeciamos una recuperación muy grande del ruido?¿Se debe esto al fin del confinamiento?
-¿vemos similitudes en los niveles acústicos entre los meses anteriores y posteriores a la cuarentena?
-¿Que franja horaria acapara un mayor porcentaje del ruido?
-¿El ruido en la noche ha aumentado respeceto al año pasado?
