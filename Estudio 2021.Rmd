...
out_3sigma %>% kable(caption = "Valores por encima y por debajo") %>%kable_styling()
...
out_percentil %>% kable(caption = "Valores por encima y por debajo") %>%kable_styling()
...
out_boxplot %>% kable(caption = "Valores por encima y por debajo") %>%kable_styling()
...
out_hampel %>% kable(caption = "Valores por encima y por debajo") %>%kable_styling()
```
...
```{r}
datos %>% group_by(level) %>% summarise(num=n(), porcentaje=num/nrow(datos)) %>% kable(caption = "Tabla porcentajes segun nivel de ruido") %>% kable_styling()
```
