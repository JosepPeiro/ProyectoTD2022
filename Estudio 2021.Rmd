...
Aparte de los outliers, estos graficos sorprender por otro detalle, y es que la forma de los violines es diferente en los distintos sonidos.
Cabría esperar que la dirtribución por la noche fuera menor, y de hecho la distribucion se centra en un valor más pequeño. Pero los outliers alcanzan valores muy similares a los que se alcanzan por la mañana.
Y luego, los valores de min y morning se centran mucho en valores concretos, mientras que los de nigth estan muchísimo más dispersos. También se centran en un valor, pero la dirtribución, restando outliers es muy simétrica.

Ahora vamos a ver el mismo grafico que antes en horizontal.
```{r}
library(gridExtra)
...
Vamos a volver a mirar si con más datos encontramos la misma relacion que antes o ahora, acabado el estado de alarma cambia algun asunto
...
```{r}
datos %>% group_by(level) %>% summarise(num=n(), porcentaje=num/nrow(datos))
```
